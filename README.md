...
La palette est centralisée dans les variables `:root` de `public/styles.css` : charbon `#242426`, blanc `#FFFFFF`, gris très clair `#F4F4F5`, paragraphes `#48484D` et séparateurs `#DCDCE0`. Navigation et accueil sont blancs ; le pied de page définit les couleurs inversées pour ses textes et focus. Les paragraphes utilisent les tokens de taille 16–17 px. Les couleurs d’erreur et de confirmation ont leurs propres variables. Le générateur de l’image de partage lit directement ces mêmes variables ; régénérer l’image avec `npm run assets:share` après une modification. Le favicon SVG et la couleur de thème du navigateur sont également assortis.
