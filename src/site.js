...
function header(path) {
  return `<header class="site-header forest-surface"><div class="container header-inner"><a class="logo" href="/" aria-label="Baerg Design, accueil">${logo}</a><button class="menu-toggle" aria-expanded="false" aria-controls="main-navigation"><span>Menu</span><span class="menu-lines" aria-hidden="true"></span></button><nav id="main-navigation" aria-label="Navigation principale">${navigation.map((link) => `<a href="${link.href}"${link.href === path ? ' aria-current="page"' : ''}>${link.label}</a>`).join('')}</nav><a class="header-cta" href="/contact/">Discutons de votre projet ${diagonal}</a></div></header>`;
}
function footer() {
  return `<footer class="site-footer forest-surface"><div class="container"><div class="footer-top"><a class="logo" href="/" aria-label="Baerg Design, accueil">${logo}</a><p>Du sens dans le fond.<br>Du caractère dans la forme.</p><nav aria-label="Navigation de pied de page">${navigation
    .slice(1)
...
function finalCta() {
  return `<section class="final-cta forest-surface"><div class="container cta-inner"><div>${eyebrow('ET SI ON EN PARLAIT ?')}<h2>Votre idée, <em>notre prochain échange.</em></h2><p>Un projet précis ou une première intuition. Tout commence par une conversation.</p></div><a class="button button-light" href="/contact/">Parler de votre projet${diagonal}</a></div></section>`;
}
function studioArt() {
  return `<figure class="studio-art"><div class="art-scene" aria-hidden="true"><div class="art-grid"></div><div class="art-note">FORME & FONCTION<br>EN ÉQUILIBRE.</div><div class="brand-sheet"><div class="sheet-top"><span>STUDIO WEB</span><span>ÉDITION 01</span></div><div class="brand-signature">${logo}</div><div class="sheet-bottom"><span>UNE AUTRE FAÇON<br>DE PRENDRE FORME.</span>${star}</div></div><div class="browser-sheet"><div class="browser-bar"><span class="browser-dots"><i></i><i></i><i></i></span><span>baerg / l’atelier</span>${diagonal}</div><div class="mini-site"><div class="mini-header"><span>BAERG DESIGN</span><span>LE STUDIO &nbsp; ↗</span></div><div class="mini-content"><div><p>Le sens<br>du <em>détail.</em></p><span>DES IDÉES AU WEB.</span><span class="mini-button">Découvrir le studio ↗</span></div><div class="sculpture"><div class="arch arch-one"></div><div class="arch arch-two"></div><div class="arch arch-three"></div><div class="sculpture-base"></div></div></div><div class="mini-footer"><span>CONCEPTION — DÉVELOPPEMENT</span><span>01 / 04</span></div></div></div><div class="art-swatch"><span></span><span></span><span></span></div><span class="art-caption">L’ESSENTIEL, BIEN PENSÉ.</span></div><figcaption><span>Un aperçu de notre univers</span><span>Identité du studio ${diagonal}</span></figcaption></figure>`;
}
...
export function renderPage(path) {
  ...
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${e(page.title)}</title><meta name="description" content="${e(page.description)}"><meta name="robots" content="${indexable ? 'index, follow' : 'noindex, nofollow'}"><meta name="theme-color" content="#173D32"><meta property="og:type" content="website"><meta property="og:locale" content="fr_CH"><meta property="og:site_name" content="Baerg Design"><meta property="og:title" content="${e(page.title)}"><meta property="og:description" content="${e(page.description)}">${origin ? `<link rel="canonical" href="${e(origin + path)}"><meta property="og:url" content="${e(origin + path)}"><meta property="og:image" content="${e(origin)}/images/og.png"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta property="og:image:alt" content="Baerg Design — Des sites web avec du fond et du caractère.">` : ''}<meta name="twitter:card" content="summary_large_image"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><link rel="preload" href="/fonts/cormorant-regular.woff2" as="font" type="font/woff2" crossorigin><link rel="preload" href="/fonts/dm-sans.woff2" as="font" type="font/woff2" crossorigin><link rel="preload" href="/fonts/pinyon.woff2" as="font" type="font/woff2" crossorigin><link rel="stylesheet" href="/styles.css"><script src="/app.js" defer></script></head><body><a class="skip-link" href="#main">Aller au contenu</a>${header(path)}<main id="main">${page.render()}</main>${footer()}</body></html>`;
}
