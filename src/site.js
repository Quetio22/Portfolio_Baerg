...
function header(path) {
  return `<header class="site-header"><div class="container header-inner"><div class="header-brand"><a class="logo" href="/" aria-label="Baerg Design, accueil">${logo}</a><button class="theme-toggle" type="button" aria-label="Mode sombre" aria-pressed="false" title="Passer en mode sombre" hidden><svg class="theme-moon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="M20.5 14A8.5 8.5 0 0 1 10 3.5 8.5 8.5 0 1 0 20.5 14Z"/></svg><svg class="theme-sun" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><circle cx="12" cy="12" r="4"/><path d="M12 2v2m0 16v2M2 12h2m16 0h2M5 5l1.5 1.5m11 11L19 19M5 19l1.5-1.5m11-11L19 5"/></svg></button></div><button class="menu-toggle" aria-expanded="false" aria-controls="main-navigation"><span>Menu</span><span class="menu-lines" aria-hidden="true"></span></button><nav id="main-navigation" aria-label="Navigation principale">${navigation.map((link) => `<a href="${link.href}"${link.href === path ? ' aria-current="page"' : ''}>${link.label}</a>`).join('')}</nav><a class="header-cta" href="/contact/">Discutons de votre projet ${arrow}</a></div></header>`;
}
function footer() {
...
function projectPreview() {
  const available = projects.filter((project) => project.published).slice(0, 3);
  if (!available.length) return projectList();
  return `<div class="project-previews">${available.map((project) => `<a class="project-preview" href="/realisations/#${e(projectAnchor(project))}" aria-label="Découvrir ${e(project.name)} dans nos réalisations"><img src="${e(assetUrl(project.image))}" alt="${e(project.imageAlt)}" width="${Number(project.width)}" height="${Number(project.height)}" loading="lazy" decoding="async"><div class="preview-caption"><h3>${e(project.name)}</h3><span class="project-nature">${e(project.nature)}</span><span class="preview-arrow" aria-hidden="true">${arrow}</span></div></a>`).join('')}</div>`;
}
function projectList(limit) {
  const available = projects.filter((project) => project.published).slice(0, limit);
  if (!available.length)
    return `<div class="projects-empty"><p>Aucun projet publié pour le moment. Les prochaines présentations détailleront le besoin, les écrans et nos choix de conception.</p></div>`;
  return `<div class="project-grid">${available.map((project) => `<article class="project" id="${e(projectAnchor(project))}" tabindex="-1"><img src="${e(assetUrl(project.image))}" alt="${e(project.imageAlt)}" width="${Number(project.width)}" height="${Number(project.height)}" loading="lazy" decoding="async"><div class="project-copy"><div class="project-heading"><h3>${e(project.name)}</h3><span class="project-nature">${e(project.nature)}</span></div><p class="project-summary">${e(project.summary || project.problem)}</p><details class="project-details"><summary>Le projet en détail <span aria-hidden="true">+</span></summary><div class="project-description"><p><strong>Le besoin.</strong> ${e(project.problem)}</p><p><strong>Notre réponse.</strong> ${e(project.solution)}</p>${project.technologies?.length ? `<p class="project-tech">${project.technologies.map(e).join(' · ')}</p>` : ''}</div></details>${project.url ? `<a class="button project-cta" href="${e(project.url)}" rel="noopener noreferrer">${e(project.linkLabel || 'Voir le site')}${arrow}</a>` : ''}</div></article>`).join('')}</div>`;
}
function workingTogether() {
  return `<section id="notre-approche" class="approach-section" aria-labelledby="approach-title">
    <div class="container section approach-layout">
      <div class="approach-heading">
        ${eyebrow(e(approach.label))}
        <h2 id="approach-title">${e(approach.title)}<br><em>${e(approach.emphasis)}</em></h2>
        <a class="text-link" href="/a-propos/">${e(approach.linkLabel)}${arrow}</a>
      </div>
      <div class="approach-story">
        <p class="approach-introduction">${e(approach.introduction)}</p>
        ${approach.paragraphs.map((paragraph) => `<p>${e(paragraph)}</p>`).join('')}
      </div>
    </div>
  </section>`;
}
...
function work() {
  ...
  return `<section class="container page-intro compact-intro"><h1>Nos réalisations.</h1><p>Chaque projet précise son contexte, les besoins auxquels il répond et nos choix de conception.</p></section><section class="container work-section">${count ? `<div class="work-section-head"><span class="eyebrow">LE PORTFOLIO</span><span>${count} projet${count > 1 ? 's' : ''} publié${count > 1 ? 's' : ''}</span></div>` : ''}${projectList()}${count ? '<div class="work-note"><p>Des projets pour apprendre, explorer ou répondre à vos besoins.</p></div>' : ''}</section>${finalCta()}`;
}
function contact() {
...
export const pages = {
  ...
  '/a-propos/': {
    ...
    description:
      'Découvrez les deux apprentis développeurs de Baerg Design et les étapes de travail : contenus, validation des maquettes et vérification des pages.',
    render: about,
  },
  '/realisations/': {
  ...
  '/404/': {
    ...
    render: () =>
      `<section class="container not-found">${eyebrow('ERREUR 404')}<span class="error-number" aria-hidden="true">404</span><h1>Une page de côté.</h1><p>Ce lien ne mène plus au bon endroit.<br>Reprenons depuis le début.</p>${button('/', 'Revenir à l’accueil')}</section>`,
  },
};
