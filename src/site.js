...
function projectList(limit) {
  ...
  return `<div class="project-grid">${available.map((project) => `<article class="project"><img src="${e(project.image)}" alt="${e(project.imageAlt)}" width="${Number(project.width)}" height="${Number(project.height)}" loading="lazy" decoding="async"><div class="project-copy"><div class="project-heading"><h3>${e(project.name)}</h3><span class="project-nature">${e(project.nature)}</span></div><p><strong>Le besoin.</strong> ${e(project.problem)}</p><p><strong>Notre réponse.</strong> ${e(project.solution)}</p>${project.technologies?.length ? `<p class="project-tech">${project.technologies.map(e).join(' · ')}</p>` : ''}${project.url ? `<a class="button project-cta" href="${e(project.url)}" rel="noopener noreferrer">${e(project.linkLabel || 'Voir le site')}${arrow}</a>` : ''}</div></article>`).join('')}</div>`;
}
function home() {
